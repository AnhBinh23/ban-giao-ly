...
const getDiem = (sc) =>
  (sc?.diem!==undefined&&sc?.diem!=='') ? parseFloat(sc.diem) : null
...
export default function ClassDetail({ cls, students, attendance, scores, onBack }) {
  ...
  const rate  = cAtt.length
    ? Math.round(cAtt.filter(a=>a.trangThai==='Có mặt').length/cAtt.length*100) : 0
  ...
  return (
    <div className="content" style={{ paddingTop:12 }}>
      ...
      <div className="card card-blue" style={{ marginBottom:14 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
          <div>
            ...
            <div style={{ fontSize:13, opacity:.85, fontWeight:700, marginTop:4 }}>{classStudents.length} thiếu nhi</div>
          </div>
            ...
            <div key={s.id} className="list-item">
              ...
              <div className="list-info">
                ...
                <div className="list-sub">
                  GX {s.giaoXu}{s.ngaySinh?` · ${new Date(s.ngaySinh).toLocaleDateString('vi-VN')}`:''}
                </div>
              </div>
              {s.phuHuynh&&(
      ...
      {tab==='diemdanh' && (
        dates.length===0
          ? <div className="empty"><div className="empty-icon">📋</div><div className="empty-text">Chưa có điểm danh</div></div>
          : <>
      ...
      {tab==='diem' && (
        <>
          <div className="tabs">
            {KY_THI.map(k=>(
              <button key={k} className={`tab ${kyFilter===k?'active':''}`} onClick={()=>setKyFilter(k)}>{k}</button>
            ))}
          </div>
          {classStudents.length===0
            ? <div className="empty"><div className="empty-icon">📝</div><div className="empty-text">Chưa có thiếu nhi</div></div>
                  ...
                  <div key={s.id} className="score-card">
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                      ...
                      <div style={{ textAlign:'right' }}>
                        {diem!==null ? (
                          <>
                            <div style={{ fontSize:20, fontWeight:900, color:diem<5?'var(--red)':'var(--sky-dd)' }}>{diem.toFixed(1)}</div>
